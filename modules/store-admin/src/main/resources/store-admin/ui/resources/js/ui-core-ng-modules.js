...
.service('Perspectives', ['$resource', function($resource){
	return $resource(relativePath + 'services/v3/js/store-admin/api/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource(relativePath + 'services/v3/js/store-admin/api/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource(relativePath + 'services/v3/js/store-admin/api/launchpad/menu.js');
}])
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource(relativePath + 'services/v3/js/store-admin/api/launchpad/views.js')
			.query({
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: relativePath + 'services/v3/web/store-admin/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.store-admin.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.store-admin.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: relativePath + 'services/v3/web/store-admin/ui/resources/templates/sidebar.html'
	}
}])
.directive('tiles', ['Tiles', function(Tiles){
	return {
		...
		templateUrl: relativePath + 'services/v3/web/store-admin/ui/resources/templates/tiles.html'
	}
}])
